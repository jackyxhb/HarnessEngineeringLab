...
- **AGENTS.md + IDE Shims (MentorScripts):** An IDE-agnostic `AGENTS.md` file at the repository root serves as the project's ground truth, with thin IDE-specific shims (`CLAUDE.md`, `.cursorrules`, `.github/copilot-instructions.md`, `.windsurfrules`) for discovery. They durably encode architectural specs, style guides, and institutional knowledge directly into the agent's context upon startup, shifting mentorship into version-controlled code.
