...
- **Filesystem, Git & File Locking (P0-2):** Does the harness use the filesystem as durable storage with Git version control, and implement file locking or task-claiming mechanisms to prevent race conditions in concurrent work?
...
- **Inter-Agent Communication (P0-10):** Does the harness provide a messaging bus for peer-to-peer messaging, broadcast, and idle notifications between agents?
- **Portable Agent Surface (P0-11):** Do all global agent instructions live in a single IDE-agnostic `AGENTS.md` file, with IDE-specific files as thin shims?
