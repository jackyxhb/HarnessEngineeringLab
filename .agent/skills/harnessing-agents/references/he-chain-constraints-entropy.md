...
### P2-1. Automated Linters [EP-15]

#### What to Do (L2)
Deterministic rules and pre-commit hooks that automatically flag and reject non-compliant code. Enforcement must be **CI-wired** — pre-commit hooks alone can be bypassed and are insufficient as the sole gate. Every linter error must include a **teaching message**: a `↳ Fix:` line pointing to the canonical source and exact remediation, so agents self-correct on first re-attempt without human intervention.

#### Don't Do (L2-inverse)
**Prevent Agents Bypassing Hooks:** Pre-commit hooks can be skipped with `--no-verify`. Without a CI gate, violations enter the repository and require manual cleanup.

#### Options (L3–L4)
- **Action:** Wire all linters into CI — pre-commit hooks are a convenience, not the enforcement gate.
- **Action:** Add a `↳ Fix:` teaching line to every linter error pointing to the canonical source.
- **Tool:** Custom deterministic linters with teaching messages.
- **Tool:** Pre-commit hooks + CI pipeline steps that both enforce the same rules.
...
### P2-2. Dependency Enforcement [EP-15]

#### What to Do (L2)
Structural tests that mechanically restrict which architectural layers the agent can import from or modify. In documentation-first repositories, applies equally to **document layer hierarchies**: canonical docs (`framework/`) are the source of truth; derived docs (`research/`) must not contradict them — enforced by structural lint checks validating pillar labels, terminology, and feature counts against canonical definitions.
...
### P2-3. AI Auditors & Collaboration Channels [EP-16]
...
### P2-4. Bounded Autonomy & Access Control [EP-17]
...
### P2-5. Upstream Intake Gate [EP-14]
...
### P3-1. Scheduled Cleanups [EP-18]

#### What to Do (L2)
Dedicated cleanup agents running on schedules to catch code that slipped past earlier checks. The implementation mechanism must be a **CI cron trigger** (e.g., weekly) — a manually-invoked workflow does not qualify as scheduled cleanup. Output is a discrete report or issue per GC category. In MAS, also reconciles overlapping or conflicting changes from concurrent agent teams.
