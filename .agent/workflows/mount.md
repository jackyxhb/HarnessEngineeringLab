...
A feature is considered "mounted" when it has physical artifacts (files, directories), recall hooks in canonical agent entry points (`AGENTS.md`), and a maintenance workflow to sustain it over time.
...
2. **Review Canonical Definitions:** Read the feature's modular file in `framework/features/` and the released skill surface in `.agent/skills/harnessing-agents/` when the feature changes how the skill navigates, audits, or outputs artifacts. Pay close attention to its "Improvement Policy," "Actions," and "Tools".
3. **Design the Live Infrastructure:** Determine the physical requirements for the feature:
   ...
   - What are the required "Recall Hooks" that force an agent to interact with this feature? (e.g., instructions in `AGENTS.md`).
...
3. **Inject Recall Hooks (`AGENTS.md`):** Modify `AGENTS.md` to explicitly instruct agents on _when_ and _how_ to use this new feature. Hook placement is critical: put it where agents reading `AGENTS.md` cannot miss it.
