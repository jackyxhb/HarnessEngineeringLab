...
This workflow processes each document in the `comments/` folder one by one, comparing it against the canonical HESkill framework documents, finding conflicts, and fixing them.

## Prerequisites

1. Read the HESkill source-of-truth documents:
   - `HESkill/Core Features for SAS.md` — the canonical **3-Pillar + 1-Foundation** framework for Single Agent Systems
   - `HESkill/Core Features for MAS.md` — the canonical framework for Multi-Agent Systems
...
1. List all files in the `comments/` folder.

2. For EACH document in `comments/`, process ONE AT A TIME in sequence:

   a. Read the document fully.

   b. Compare it against the HESkill canonical framework. Look for these specific conflict types:
   ...
   d. If the document is a **principles/essay/example document** (doesn't define framework structure), check that any referenced concepts are consistent with HESkill terminology. These documents typically have no conflicts — note them as clean and move on.
