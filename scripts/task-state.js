...
function startState(options) {
  ...
  const state = {
    ...
    requirement_ids: options['requirement-ids'] ? options['requirement-ids'].split(',').filter(Boolean) : [],
    status: 'active',
...
function markStep(options) {
  ...
  const completedSteps = Number.parseInt(options['completed-steps'] || `${state.completed_steps + 1}`, 10);
  ...
  if (options.label) {
    state.completed_step_labels = [...new Set([...(state.completed_step_labels || []), options.label])];
  }
  if (options.summary) {
    state.state_summary = options.summary;
  }
...
function completeState(options) {
  ...
  state.completed_steps = Number.parseInt(options['completed-steps'] || `${state.expected_steps}`, 10);
  state.updated_at = currentTimestamp();
  state.last_heartbeat_at = state.updated_at;
  if (options.summary) {
    state.state_summary = options.summary;
...
function main() {
  ...
  switch (command) {
    ...
    case 'show':
      ...
      break;
    default:
      throw new Error('Usage: node scripts/task-state.js <start|heartbeat|step|fail|complete|reset|show> [--key=value]');
  }
}
