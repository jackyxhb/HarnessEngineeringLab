...
const TASK_STATE_FILE = path.join(__dirname, '..', '.harness', 'task-state.json');
const PERMISSIONS_FILE = path.join(__dirname, '..', '.harness', 'agent-permissions.json');
const MCP_CAPABILITIES_FILE = path.join(__dirname, '..', '.harness', 'mcp-capabilities.json');
...
function buildAlerts({ latestLog, taskState, escalationEvents, reportTimestamp }) {
  const alerts = [];
  const staleHeartbeatMinutes = differenceInMinutes(taskState?.last_heartbeat_at, new Date(reportTimestamp));
  const openEscalations = escalationEvents.filter(event => event.status !== 'resolved');

  if (latestLog && ['fail', 'error', 'warn'].includes(latestLog.result)) {
    alerts.push({
      severity: latestLog.result === 'fail' || latestLog.result === 'error' ? 'high' : 'medium',
      type: 'latest-log-result',
      message: `Latest logged action ended with result=${latestLog.result}`
    });
  }

  if (taskState && !['completed', 'idle', 'absent'].includes(taskState.status)) {
    alerts.push({
...
function renderDashboard(report) {
  const recentActions = report.recent_actions.length > 0
    ? report.recent_actions
        .map(action => `- ${action.timestamp} | ${action.action} | ${action.result} | ${action.target}`)
        .join('\n')
  ...
  const taskState = report.task_state.status === 'absent'
  ...
  return [
    ...
    '## Result Breakdown',
    '',
    ...Object.entries(report.results_by_type).map(([result, count]) => `- ${result}: ${count}`),
    '',
    '## Task State',
    '',
    taskState,
...
function generateReport() {
  ...
  const resultsByType = {};
  const durations = [];
  const errors = logs.filter(log => ['error', 'fail'].includes(log.result)).length;
  ...
  const report = {
    ...
    escalation_summary: {
      ...
      open_events: escalationEvents.filter(event => event.status !== 'resolved').length,
    ...
    system_health: {
      ...
      task_state_schema_present: fs.existsSync(path.join(__dirname, '..', '.harness', 'task-state.schema.json')),
      escalation_rules_present: fs.existsSync(path.join(__dirname, '..', '.harness', 'escalation-rules.json'))
    },
