...
- Removed stale `Mode 3` labels from `.agent/skills/harnessing-agents/SKILL.md` and `templates/HE-FEATURE-LOOKUP.md`. The skill has had only two modes (`full`, `feature`) since the quick-scan removal; the `Mode 3` references were orphaned from an earlier three-mode version and contradicted the `How to Use (2 Modes)` heading.
