...
## 1. Functional Maturity (Based on the "Three Pillars")

You can divide the project's maturity based on how many of the core "pillars" of harness engineering have been successfully implemented:

* **Level 1 - Context Engineering:** The harness successfully delivers the right information to the agent at the right time. This includes setting up static context (like `AGENTS.md` and architecture specs), dynamic context (like real-time CI/CD status), and ensuring the repository acts as the single source of truth.
* **Level 2 - Architectural Constraints:** The harness moves beyond simply giving the agent instructions and begins **mechanically enforcing boundaries**. This level is achieved when deterministic linters, structural tests, and LLM-based auditors are actively preventing the agent from exploring dead ends or violating dependency rules.
* **Level 3 - Entropy Management (Garbage Collection):** The highest functional level, where the system maintains its own health over time. This is achieved by deploying periodic cleanup agents that run on schedules to automatically fix documentation drift, resolve circular dependencies, and enforce coding patterns.
